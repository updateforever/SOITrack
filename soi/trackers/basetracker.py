        ...
        for f, img_file in enumerate(img_files):
            ...
            if f == 0:
                ...
                if track_vis:
                    ...
                    # text
                    font_face = cv.FONT_HERSHEY_SIMPLEX
                    cv.putText(frame_disp, 'No.%06d' % (f), (0, 20), font_face, 0.8, (0, 255, 0), 2)
                    if (anno[f, :] != np.array([0, 0, 0, 0])).all():
                    ...
                    cv.rectangle(frame_disp, (state[0], state[1]), (state[2] + state[0], state[3] + state[1]), (0, 0, 255), 2)  # (0, 0, 255)是红色
                    ...
                    for i, box in enumerate(frame_candidate_data['candidate_boxes']):
                        ...
                        temp_state[2] = width - temp_state[0] if temp_state[0] + temp_state[2] > width else temp_state[2]
                        temp_state[3] = height - temp_state[1] if temp_state[1] + temp_state[3] > height else temp_state[3]
                        temp_iou = iou(track_result, box.cpu().numpy(), bound=bound)
                        if temp_iou > 0.6:
                            continue
                        # 画出候选框 虚线灰色
                        cv.rectangle(frame_disp, (temp_state[0], temp_state[1]), (temp_state[2] + temp_state[0],
                                                                                    temp_state[3] + temp_state[1]), (200, 200, 200), 2) # 黄色(0, 255, 255)  
                ...
                if heatmap_vis:
                    ...
                    font_face = cv.FONT_HERSHEY_SIMPLEX
                    cv.putText(heat_map_search_img, 'No.%06d' % (f), (10, 20), font_face, 0.8, (0, 255, 0), 2)
                ...
                if masked_image:
                    ...
                    # 如果没有其他候选框，直接保存原图并跳过后续操作
                    # if len(frame_candidate_data['candidate_boxes']) <= 1:
                    #     cv.imwrite(masked_save_path, image)
                    #     continue  # 跳过当前帧的后续 Mask 逻辑
                    # 初始化 mask 统计信息
                    masked_boxes = []  # 用于记录 mask 的帧号和信息

                    # 当前帧的 Mask 操作
                    mask_count = 0  # 当前帧的 mask 计数
                    masked_frame = image.copy()  # 拷贝原图用于 mask 操作

                    for box in frame_candidate_data['candidate_boxes']:
                        temp_state = [int(s) for s in box.squeeze(0)]
                        # temp_iou_gt = iou(current_gt, box.cpu().numpy(), bound=bound)
                        temp_iou_pred = iou(track_result, box.cpu().numpy(), bound=bound)

                        # Mask 条件：与 gt 或预测框的 IoU 超过阈值
                        if temp_iou_pred < 0.7:  # temp_iou_gt < 0.7 or
                            # 获取候选框的坐标范围
                            x1, y1, x2, y2 = temp_state[0], temp_state[1], temp_state[0] + temp_state[2], temp_state[1] + temp_state[3]
                            
                            # 在图像上进行 Mask 操作，将区域设置为黑色 (0, 0, 0)
                            masked_frame[y1:y2, x1:x2] = (0, 0, 0)  # 或 (128, 128, 128) 设置为灰色
                    ...
                    # 恢复 GT 区域的像素
                    masked_frame[gt_y1:gt_y2, gt_x1:gt_x2] = image[gt_y1:gt_y2, gt_x1:gt_x2]  # 还原原始像素

                    # 保存 Mask 后的图像
                    cv.imwrite(masked_save_path, masked_frame)

                    # 记录 Mask 信息
                    if mask_count > 0:
                        masked_boxes.append({"frame": f, "count": mask_count})

                    # 更新全局统计信息
                    total_masks = sum([entry["count"] for entry in masked_boxes])

                    # 保存统计信息到 JSON 文件
                    masked_json_path = "{}/masked_info.json".format(img_path)
                    if os.path.exists(masked_json_path):
                    ...
                    # 更新 JSON 数据
                    json_data.append({"seq_name": seq_name, "masked_info": masked_boxes, "total_masks": total_masks})
