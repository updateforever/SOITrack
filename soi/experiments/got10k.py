...
class ExperimentGOT10k(object):
    ...
    def __init__(self, root_dir, save_dir, subset='val', list_file=None, use_dataset=True, th=0.1):
                ...
                for s, (_, anno, meta) in enumerate(self.dataset):
                    ...
                    if name is 'uav_kt':
                        record_files = [os.path.join(
                            ...
                            '%s_001.txt' % seq_name)]
                    elif name is 'keeptrack':
                        record_files = [os.path.join(
                            '/mnt/first/hushiyu/SOT/GOT-10k/result/results/KeepTrack/test',
                            'KeepTrack_%s_1.txt' % seq_name)]  # KeepTrack_GOT-10k_Test_000107_1.txt
                    else:
    ...
    def sequences_select(self, tracker_names, save_dir, screen_mode='MaxPooling', th=0.1):
        assert isinstance(tracker_names, (list, tuple))
        print('th is:' + str(th))
        if not os.path.isdir(save_dir):
            os.makedirs(save_dir)
        name = self.dataset.name
        subset = self.dataset.subset
        info_file = os.path.join(save_dir, 'data', name, subset, screen_mode + 'th_{}'.format(th),
                                 'list.txt')
        seq_names = []

        for s, (_, anno, meta) in enumerate(self.dataset):  # 序列遍历
            seq_name = self.dataset.seq_names[s]
            seq_trackers_judge = 0
            ratio = {}
            seq_tc_num = {}
            ...
            for i, tracker in enumerate(tracker_names):  # tracker遍历
                ...
                num_csore_json_file = os.path.join(save_dir, 'score_map_json', tracker, self.dataset.name, subset,
                                                   'score_map',
            ...
            for j in range(len(anno) - 1):
                if meta['absence'][j+1] == 1:
                    print('soi %d is absent frame' % j)
                    continue
                a = 0
            ...
            if len(soi_frames) > 0:  # 存在soi
                self.choose_seq_for_soi(seq_name, soi_frames, save_dir, screen_mode, th)  # 写个脚本统计下帧数占比情况吧
                # append
                seq_names.append(seq_name)
